...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in {"create", "create_payment_intent"}:
    ...
    def get_queryset(self):
        ...
        if self.action == "create_payment_intent":
            return qs  # AllowAny: guests need to pay for the order they just placed
    ...
    def create_payment_intent(self, request, pk=None):
        order = self.get_object()
        try:
            data = create_stripe_payment_intent(order)
    ...
    def mark_paid(self, request, pk=None):
        ...
        if order.payment_method == Order.PaymentMethod.CARD:
            return Response({"detail": "Card orders are marked paid automatically via Stripe."}, status=400)
...
class StripeWebhookView(APIView):
    """
    Receives Stripe events (payment_intent.succeeded / .payment_failed).
    Exempt from both DRF auth and CSRF: Stripe can't supply either, and the
    request is authenticated instead via its signature (verified inside
    handle_webhook_event using STRIPE_WEBHOOK_SECRET).
    """
    ...
    def post(self, request):
        try:
            handle_webhook_event(request.body, request.headers.get("Stripe-Signature", ""))
        except PaymentError as exc:
            return Response({"detail": str(exc)}, status=400)
        except Exception:
            return Response({"detail": "Invalid webhook payload or signature."}, status=400)
